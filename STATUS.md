...
| C2 photo import | **Finale.** Laptop hub only (decision of 27 Sep). Built: `ocr.py` (Tesseract with `hin.traineddata`, photo straightened, greyed, enlarged, deleted after reading) → `POST /curriculum/photo` → the text in the lesson-import box for the teacher to check (flag `PHOTO_IMPORT`, off until measured). Ten test pages of the team's own Hindi to print and photograph (`docs/samples/ocr_pages/`), scored by `bench/ocr_eval.py`. Tesseract v5.5.3 with `hin` installed (27 Sep). Clean rendered pages (an upper bound, not photos): CER 5.0 %. **Finale (team decision, 27 Sep): the photo test is skipped for the submission; photo import stays off and no OCR accuracy is claimed.** **No ML Kit on the tablet**: it sends usage metrics to Google when online, against the offline and privacy design | `ocr.py`, `docs/sources.md#tesseract`, `#mlkit-text` |
...
| **Finale plan:** a character-based voice without espeak-ng (sherpa-onnx's Piper path includes espeak-ng, GPL-3.0; the tablet already uses sherpa-onnx, so the APK is distributed under GPL-3.0 terms — see the APK licence row at the top) | planned | `THIRD_PARTY_LICENSES.md` |
