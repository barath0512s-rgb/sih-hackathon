...
| A3 round-trip flag | **Built, flag on (`ROUNDTRIP_CHECK`), a weak signal.** A model translation is translated back; round-trip chrF below 34.5 → "⚠️ मूल वक्ता से जाँचें", no auto-play, nearest verified sentence. IN22-Conv test half: precision 0.446, recall 0.558 for the worst quarter of translations (random flagging would have precision about 0.28); it flags about a third of model output. Typed translations and content packs only; voice replies skip it (a second model pass). New content pack: 37 of 110 model lines flagged; the tablet shows the flag from the pack | `eval/results/roundtrip_flag.md`, `tests/test_roundtrip.py` |
...
| C2 photo import | **Blocked.** Laptop: Tesseract (Apache-2.0) is not installed; installing it is the team's step (Windows installer from UB Mannheim, as the Tesseract docs direct, plus `hin.traineddata`). Tablet: ML Kit Text Recognition v2 (Devanagari, bundled, offline) is under the Google APIs Terms of Service and **sends usage metrics to Google when online**: not added without the team's approval. The text → lesson step already exists (lesson import) | `docs/sources.md#tesseract`, `#mlkit-text` |
| C3 Mundari translation | **Blocked on the GPU run** of `notebooks/mundari_lora.ipynb` (and the Kaggle competition rules accepted by the team). The official MMLoSo test set has no public references | `docs/sources.md#mmloso` |
| C4 own Santali voice | **Blocked on the GPU run** of `notebooks/santali_voice.ipynb` | notebook |
